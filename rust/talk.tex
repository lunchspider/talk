...
\begin{frame}
    ...
    The definition of good/bad is judged by multiple factors, there is usually 
    a first pass for each image for model inference, then duplicate detection and then selection.
    Usually, we have multiple kind of pipeline that runs different models depending on the type of image
\end{frame}
